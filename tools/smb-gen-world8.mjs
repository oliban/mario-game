...
// walked past that pipe and set 8-1 down in the middle of three koopas.
...
// ---------------------------------------------------------------------- 8-1
{
  ...
  const back = backExit(L, rows, 8, bonusPagesFor('8-1')[0], wp.x + 24);
  const body = `
...
// ---------------------------------------------------------------------- 8-2
{
  ...
  const sp = findSpawn(rows);
  const wp = L.meta.warpPipe;
  const back = backExit(L, rows, 8, bonusPagesFor('8-2')[0], wp.x + 24);
  const body = `
